...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($email_validate == "exists") {
        ...
        if ($stmt) {
            ...
            if ($stmt->execute()) {
                header("location: ../view-admin.php?success=Admin created successfully");
            } else {
                header("location: ../view-admin.php?error=Error" . $stmt->error);
            }
            $stmt->close();
        } else {
            header("location: ../view-admin.php?error=Error" . $conn->error);
        }
    }
    $conn->close();
} else {
    header("location: ../view-admin.php?error=Error! Invalid request method");
}
